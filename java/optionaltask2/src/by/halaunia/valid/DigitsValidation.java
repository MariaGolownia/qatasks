...
public class DigitsValidation {
    ...
    public static void checkArrIfOfNumbers(int[] args) throws IncorrectDataException {
        if (args == null) {
            throw new IncorrectDataException("Args of string is null!");
        } else {
            for (int i = 0; i < args.length; i++) {
                checkIfNumber(Integer.toString(args[i]));
            }
        }
    }

    public static void checkIfMaxBiggerMin(Integer minValue, Integer maxValue) throws IncorrectDataException {
        if (maxValue <  minValue) {
            throw new IncorrectDataException("Max number cannot be less than min number!");
        }
    }
}
